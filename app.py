...
def query_agent():
    ...
    try:
        ...
        # Check for specific tool outputs to create structured responses
        tool_output = None
        tool_used = None
        if 'intermediate_steps' in result and result['intermediate_steps']:
            action, tool_output = result['intermediate_steps'][-1] # Get the last tool call
            tool_used = action.tool

        # --- NEW LOGIC for structured creation questions ---
        if tool_used in ['start_ticket_creation', 'set_ticket_field'] and isinstance(tool_output, dict) and 'options' in tool_output:
            response_data = {
                ...
                "content": tool_output,
                "raw_output": tool_output.get('question', '') # Use the question for history
            }
        # --- END NEW LOGIC ---
