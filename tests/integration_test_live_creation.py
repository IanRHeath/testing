...
# Make sure you run this test from the root of your project
from jira_tools import create_ticket_tool, JIRA_CLIENT_INSTANCE

class TestLiveTicketCreation(unittest.TestCase):

    # NOTE: We are NOT mocking 'create_jira_issue' here. This test will make a real API call.
    ...
    @patch('builtins.input', side_effect=['', 'yes']) # Automate "Press Enter" and "confirm creation" prompts
    @patch('jira_tools.get_summary_similarity_score', return_value=2) # Assume no duplicates are found to streamline the test
    @patch('jira_tools.search_jira_issues', return_value=[]) # Assume no candidates are found
    def test_live_ticket_creation_and_deletion(self, mock_search, mock_similarity, mock_input, mock_file):
        ...
        """
        # --- Test Setup ---
        # A variable to hold the key of the created ticket for cleanup
        new_ticket_key = None
        
        # Define the arguments for the new ticket
        args = {
            ...
            "severity": "Low",
            "project": "PLAT" # Ensure this project exists and you have create/delete permissions
        }
        ...
        try:
            ...
            # Extract the new ticket key from the result string for cleanup
            ...
            # --- Cleanup Phase ---
            # This block will run even if the assertions in the 'try' block fail.
